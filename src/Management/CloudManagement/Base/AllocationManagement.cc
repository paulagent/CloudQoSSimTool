...
void AllocationManagement::initialize() {
...
void AllocationManagement::updateVmManagementStructures(
        vector<int> selected_nodes, int vmPid, int userID) {
    // Define ..
...
// Update nodes structure
    ...
    while (selected_nodes.size() > 0) {
        ...
        initialNode = *(storageNodesOccupation.begin()
...
vector<AbstractNode*> AllocationManagement::getStorageNodesOfVM(int userIdx,
        int pId) {
...
vector<AbstractNode*> AllocationManagement::deleteVMfromStorageNodes(
        int userIdx, int pId) {
...
AbstractNode* AllocationManagement::getVmAllocation(int uId, int pId) {
    ...
    for (i = 0; (i < userVmData.size()) && (!vmFound); i++) {
        ...
        if (userAl->user == uId) {
            for (j = 0; (j < userAl->vmResourcesInfo.size()) && (!vmFound);
                    j++) {
                ...
                if (vmAl->vm->getVMID() == pId) {
                    ...
                    vmFound = true;
                    // Get the allocation
                    node = getNodeByIndex(vmAl->vm->getVM()->getNodeSetName(),
                            vmAl->vm->getVM()->getNodeName());
