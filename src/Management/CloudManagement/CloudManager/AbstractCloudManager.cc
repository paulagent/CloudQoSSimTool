...
// uvic add
        //we should also remove it from running vm
        int id = vm->getId();
       ...
       if (req_copy->is_freezed)
       {
           cout<< "************************VM is freezed---> We should unfreez it*****************************************"<<endl;
           base->setOperation(REQUEST_UNFREEZE_VM);
           RequestsManagement::userSendRequest(base);
    ...
    if (vm != NULL) {
     ...
     //   cout << "node->getNumCores()------" << node->getNumCores() << endl;
        ...
        if (nodeVL->testLinkVM(vm->getNumCores(), vm->getMemoryCapacity(),
            ...
            linkVM(nodeVL, vm);
            cout << "unfreeze vm -> vm->turnon after link vm" << endl;
            //vm->turnOn();
            return false;
            ...
            //    cout << "------------------j  :" << j << endl;
             ...
             //       cout << "vmr->end_time----->"<<vmr->end_time<< endl;
                    if (t >= vmr->end_time)   // we need to freeze vm
                        ...
                        vmr->vm->setPendingOperation(PENDING_UNFREEZ); //
...
/*                      printf(
                        ...
                        //    cout << "unfreeze vm -> vm->turnon after link vm" << endl;
                          //  vm->turnOn();
                            ...
                            started_VM->hostNodeVL = nodeVL;
                            started_VM->vm ->setPendingOperation(NOT_PENDING_OPS);
