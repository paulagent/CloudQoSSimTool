...
void HWEnergyInterface::e_changeState (string state, unsigned int componentIndex){
...
int HWEnergyInterface::e_getStatesSize (unsigned int componentIndex){
    ...
    if ((*(meterStates.begin() + componentIndex)) != NULL){
...
simtime_t HWEnergyInterface::e_getStateTime (string state, unsigned int componentIndex){
...
simtime_t HWEnergyInterface::e_getActualStateTime (string state, unsigned int componentIndex){
...
double HWEnergyInterface::e_getConsumptionValue (string state, unsigned int componentIndex){
    double res;
...
float* HWEnergyInterface::getStatesVector(unsigned int componentIndex){

    float* states = NULL;

    if ((componentIndex >= 0) && (componentIndex < meterStates.size()))
...
float* HWEnergyInterface::getTotalVector(){
    ...
    bzero (nonAccumulatedTimeEntire, statesSize*numberOfComponents*sizeof(float));
...
void HWEnergyInterface::resetStatesVector(unsigned int componentIndex){
    if ((componentIndex >= 0) && (componentIndex < meterStates.size())){
...
void HWEnergyInterface::activateMemoVector(unsigned int componentIndex){
    if ((componentIndex >= 0) && (componentIndex < meterStates.size())){
