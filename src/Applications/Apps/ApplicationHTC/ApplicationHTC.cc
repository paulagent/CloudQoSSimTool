...
void ApplicationHTC::initialize(){
    MPI_Base::initialize();
}
...
void ApplicationHTC::processRequestMessage (icancloud_Message *sm){
	showErrorMessage ("This module does not recieve request messages!");
}

void ApplicationHTC::processResponseMessage (icancloud_Message *sm){
...
void ApplicationHTC::processNetCallResponse (icancloud_App_NET_Message *responseMsg){
...
void ApplicationHTC::processIOCallResponse (icancloud_App_IO_Message *responseMsg){
	...
	std::ostringstream osStream;
		...
		// Check the results...
			...
			if (currentState != HTCAPP_READING_FILE)
				throw new cRuntimeError("State error! Must be HPCAPP_MASTER_READING\n");
...
void ApplicationHTC::processCPUCallResponse (icancloud_App_CPU_Message *responseMsg){
